...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `
      SELECT 
        a.id, a.title, a.slug, a.content, a.excerpt, a.cover_image, a.status,
...
// Get single article for admin (requires auth)
...
router.post('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const db = await getDb();
    const { title, content, excerpt, category_id, cover_image, status = 'draft' } = req.body;
    ...
    const result = await db.run(
      `INSERT INTO articles (title, slug, content, excerpt, cover_image, status, user_id, category_id, published_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [title, slug, content, excerpt || null, cover_image || null, status, req.user!.id, category_id || null, publishedAt]
    );
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const db = await getDb();
    const { id } = req.params;
    const { title, content, excerpt, category_id, cover_image, status } = req.body;
